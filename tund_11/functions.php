<?php
	...
	function publicphotoslist($privacy){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy<=? AND deleted IS NULL ORDER BY id");
		echo $mysqli->error;
		$stmt->bind_param("i", $privacy);
		$stmt->bind_result($filenameFromDB,$alttextFromDB);
		$stmt->execute();
		while($stmt->fetch()){
		$notice .= "<img src='".$GLOBALS["thumbDir"].$filenameFromDB."' alt='".$alttextFromDB."'> ";
		}
		if (empty($notice)){
			$notice = "Kahjuks pilte pole";
		}
		$stmt->close();
		$mysqli->close();
		return $notice;
	...
	function privatephotoslist($privacy){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	...
	function publicphotoslistpage($privacy){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy<=? AND deleted IS NULL ORDER BY id LIMIT 1,2");
		echo $mysqli->error;
		$stmt->bind_param("i", $privacy);
		$stmt->bind_result($filenameFromDB,$alttextFromDB);
		$stmt->execute();
	...
	function latestPicture($privacy){
		...
		$stmt->execute();
		if($stmt->fetch()){
		$notice = "<img src='".$GLOBALS["picDir"].$filenameFromDB."' alt='".$alttextFromDB."' \n";
		} else {
			$notice = "<p>Kahjuks avalikke pilte pole</p>";
		}
		$stmt->close();
		$mysqli->close();
		return $notice;
